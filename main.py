...
from deepfake_detector import run as video_run  # Directly import the run function
import importlib
...
# Set maximum upload size to 20 MB
...
def upload_file():
    ...
    if file and allowed_file(file.filename):
        # Use the original extension for saving
        ...
        video_path2 = os.path.join(app.config['UPLOAD_FOLDER'], "processed_" + f"uploaded_video_{timestamp}.mp4")

        # Analyze video using the deepfake_detector module (comprehensive analysis)
        video_accuracy = video_run(video_path, video_path2)

        # Analyze audio using audio_analyzer module (feature extraction)
        ...
        # Record analysis in the blockchain
        blockchain.new_transaction(video_info['name'], video_info['video_accuracy'], video_info['audio_accuracy'], video_info['file_hash'])
        blockchain.new_block(proof=12345)  # Demo proof value
        # Log AI model metadata for transparency
